...
const myHabit = AsyncHandler(async (req, res) => {
  ...
  const ownHabit = await Habit.find({ user: "69155f9770efcfaec8197ba1" }); //.populate("user", "name email");
  console.log("own", ownHabit);
  if (!ownHabit) {
...
const isCompleteHabit = AsyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, { isComplete, stack }, "habit created successfully")
    );
});

const recentHabits = AsyncHandler(async (req, res) => {
  const recent = await Habit.find().sort({ createdAt: -1 }).limit(6);
  if (!recent) {
    throw new ApiError(401, "recent habit does not exist");
  }
  return res
    .status(200)
    .json(new ApiResponse(200, recent, "recent product founded"));
});
